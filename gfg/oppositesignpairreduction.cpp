...
Solved
Difficulty: MediumAccuracy: 50.06%Submissions: 61K+Points: 4Average Time: 30m
...
Topic Tags
Related Articles
If you are facing any issue on this page. Please le*/
  public:
    vector<int> reducePairs(vector<int>& arr) {
        ...
        {
            bool alive=true;
            while(alive&&!st.empty()&&(arr[i]<0&&st.top()>0||arr[i]>0&&st.top()<0))
            {
                int temp1=abs(arr[i]);
                int temp2=abs(st.top());
                if(temp1==temp2)
                {
                    st.pop();
                    alive=false;
